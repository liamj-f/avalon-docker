...
`frontend/src/**/*.test.{js,jsx}` is a Vitest + React Testing Library suite
— no backend, no Postgres, no browser (jsdom stands in for the DOM). It's
deliberately not exhaustive: rather than snapshotting every component, it
targets the places a regression is both easy to introduce and easy to miss
by eye — `gameData.js`'s client-side rule validation (mirrors the backend's
real validation, purely for lobby UX), `StuckPhaseNotice`'s confirm-gate
(every force-resolve action goes through it — see the design note below),
`PlayerAvatar`'s badge-priority logic, and `Chat`'s system-vs-player message
rendering.

```bash
cd frontend
npm install
npm test          # vitest run -- single pass, exits non-zero on failure
npm run test:watch  # vitest -- reruns on file change, for local development
```

Both suites run in CI on every push/PR to `main`
(`.github/workflows/backend-tests.yml` and `frontend-tests.yml`) — a
failing test fails the run and shows as a red check on the commit/PR in
GitHub, the same as any other CI failure. GitHub doesn't block a merge on
that by itself unless the repo's branch protection rules are set up to
require it; either way, nothing here auto-fixes or auto-skips a failing
test — same as the backend suite, a red run means either the code or the
test needs a real look before that work is considered done.
...
backend/
  ...
  tests/
    ...
    test_rooms.py                        # rooms.py's in-memory Room: host reassignment, mute/kick guards -- also no DB
  migrations/
...
frontend/
  ...
  vitest.config.js     # separate from vite.config.js -- the PWA plugin there has no meaning for a jsdom test run
.github/workflows/
  backend-tests.yml     # pytest against a Postgres service container
  frontend-tests.yml    # vitest, then a production build as a smoke check
...
close: mid-game, host authority never moved at all — a plain disconnect
(unlike the lobby's) never even reassigned it, so if the disconnecting
player *was* the host, every host-only action, escape hatches included,
went down with them until that specific browser tab came back.
`Room.mark_disconnected` now reassigns host immediately when this happens,
to another connected seat — same `_next_host_candidate` preference
`remove_player`'s lobby-phase reassignment already used, now shared:
prefer someone not muted (a muted player made host this way would be
stuck, since `set_muted` refuses to let a host act on their own seat in
either direction), falling back to muted-but-connected, and finally to
anyone left at all rather than leaving the room hostless. A host
deliberately handing off host (`transfer_host`) is unaffected by any of
this — that's still purely the host's own choice, lobby-only, no
preference applied.

Two more properties, both about the fact that these are irreversible
actions taken on someone else's behalf rather than the host's own move:

- **Confirmation.** Every `StuckPhaseNotice` button (the shared component
  all five panels render) requires a `confirmMessage` prop and wraps its
  `onClick` in `window.confirm(confirmMessage)` — a caller that forgot to
  pass one is a prop-types-shaped bug, not a silently-missing confirm.
  `AssassinPanel`'s is the sharpest: force-passing the assassination ends
  the game outright, so its message says so explicitly rather than reusing
  a generic "are you sure?".
- **Transparency.** A force-resolve isn't visible to anyone but the host
  who clicked it unless something says otherwise. Each of the 5 socket
  handlers now also calls `room.add_chat_message(..., system=True)` right
  after the stored procedure call succeeds (never before — a call that
  raises, e.g. because there was nothing actually stuck, posts nothing),
  describing what happened and attributed to the acting host. `Chat.jsx`
  renders `system: true` entries as a distinct italic line (`.chat-line-system`)
  instead of a normal "Name: message" bubble, so the whole table sees a
  host used an escape hatch, not just the host.
...
- **Mid-game host reassignment, force-resolve confirmations, and chat
  transparency** (the three additions right above): verified together in
  one real 5-browser-context Playwright run against a live backend/Postgres,
  not just via `test_rooms.py`'s pure-Python coverage. Played quest 1 to a
  real Success, then closed the host's (Alice's) browser context entirely
  mid-game — every other client immediately showed Bob (the next connected,
  unmuted seat) tagged `HOST`, Alice tagged `OFFLINE`, with her seat still
  present rather than removed. Quest 2's leader then proposed a team
  including Alice's now-unreachable seat; the vote stalled, and Bob's
  `VotePanel` showed its force-resolve button. Clicking it fired a real
  `window.confirm()` — captured via Playwright's `dialog` event rather than
  auto-accepted — with exactly the text `StuckPhaseNotice` was given
  (`"Force this vote through? Anyone still missing a vote will be counted
  as Approve."`); accepting it resolved the vote and posted
  `"⚡ Force-resolved the stuck vote — anyone still missing a vote was
  counted as Approve. — Bob, host"` to chat. Checked from **Charlie's**
  page — a client that took no part in the force-resolve — confirming the
  system-message line reaches the whole table, styled distinctly
  (`.chat-line-system`), not just a toast on the acting host's own screen.
- **Frontend test suite**: `npm test` (Vitest + jsdom) — 24 tests across 4
  files, all passing: `gameData.test.js` (client-side settings validation),
  `StuckPhaseNotice.test.jsx` (the confirm-then-call / cancel-then-no-call /
  disabled-does-nothing behavior every force-resolve button depends on),
  `PlayerAvatar.test.jsx` (Excalibur-badge-beats-nomination-badge priority,
  offline styling, selectable/disabled gating), and `Chat.test.jsx`
  (system vs. normal message rendering, muted-hides-the-form). `npm run
